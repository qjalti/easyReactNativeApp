...
export default function HomeScreen() {
    ...
    if (value) {
      ...
      setMPM(
        new Intl.NumberFormat("ru-RU")
          .format(Number(METEORS_PER_MINUTE))
          .replace(",", "."),
      );
      setMPS(
        new Intl.NumberFormat("ru-RU")
          .format(Number(METEORS_PER_SECOND))
          .replace(",", "."),
      );
    }
  };
  ...
  const calculateDepth = (value: string) => {
    ...
    if (value) {
      ...
      setDepth(
        new Intl.NumberFormat("ru-RU").format(Number(DEPTH)).replace(",", "."),
      );
    }
  };
  ...
  const calculateLightningDistance = (value: string) => {
    ...
    if (value) {
      ...
      setDTL(
        new Intl.NumberFormat("ru-RU")
          .format(Number(DISTANCE_TO_LIGHTNING))
          .replace(",", "."),
      );
    }
  };
  ...
  return (
    <ScrollView>
      ...
      <ThemedView style={styles.stepContainer}>
        ...
        />
        <ThemedText>1 метеор каждые: {meteorsPerSecond}s</ThemedText>
        <ThemedText>1 метеор каждые: {meteorsPerMinute}m</ThemedText>
      </ThemedView>
      <ThemedView style={styles.stepContainer}>
        ...
        <TextInput
          ...
          style={{ color: inputTextColor }}
        />
        <ThemedText>Глубина: {depth}m</ThemedText>
      </ThemedView>
      <ThemedView style={styles.lastStepContainer}>
        ...
        <TextInput
          ...
          style={{ color: inputTextColor }}
        />
        <ThemedText>Растояние до молнии: {distanceToLightning}m</ThemedText>
      </ThemedView>
    </ScrollView>
  );
}
